...
fn test_eip712_domain_separator_generation() {
    ...
    client.initialize(&admin, &minter_manager, &token_contract, &oracle, &treasurer, &verifier_pubkey, &100, &domain_name, &domain_version);
...
fn test_withdraw_verifier_key_management() {
    ...
    let domain_name = soroban_sdk::String::from_str(&env, "withdraw");
    let domain_version = soroban_sdk::String::from_str(&env, "1");
    client.initialize(&admin, &minter_manager, &token_contract, &oracle, &treasurer, &initial_verifier_pubkey, &100, &domain_name, &domain_version);
...
fn test_withdraw_invalid_signature_length() {
    ...
    client.withdraw(&user, &target_amount, &nav, &request_hash, &timestamp, &invalid_signature);
}
...
fn test_withdraw_with_mock_pubkey() {
    ...
    client.withdraw(&user, &target_amount, &nav, &request_hash, &timestamp, &signature);
}
